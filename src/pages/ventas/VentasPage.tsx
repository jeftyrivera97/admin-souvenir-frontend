import {
  IndexVentasStatisticsComponent,
  IndexVentasDataTable,
  MonthDateInput,
} from "@/components";
...
export const VentasPage = () => {
  const pagination = useVentaStore((s) => s.pagination);
  const fetchVentas = useVentaStore((s) => s.fetchVentas);
  const setSelectedMonth = useVentaStore((s) => s.setSelectedMonth);
  ...
  const selectedMonth =
    useVentaStore((s) => s.meta.month) ?? getCurrentYearMonth();
  ...
  useEffect(() => {
    ...
    setSelectedMonth(month);
    // ensure data loaded
    fetchVentas(1, pagination.limit, month).catch(() => {});
